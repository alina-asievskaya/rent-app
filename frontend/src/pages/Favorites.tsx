  ...
  const getHouseOwnerInfo = async (houseId: number): Promise<number | null> => {
    try {
      ...
      if (response.ok) {
        ...
        if (property?.ownerId) return property.ownerId;
      }
      return null;
    } catch (error) {
          ...
          <div className="container-favorit">
            <div className="favorites-empty-state-favorit">
              ...
              <div className="empty-state-actions-favorit">
                <Link to="/login" className="btn-primary-favorit">Войти</Link>
                <Link to="/register" className="btn-secondary-favorit">Зарегистрироваться</Link>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="favorites-tips-favorit">
                ...
                <div className="tips-grid-favorit">
                  <div className="tip-card-favorit">
                    <div className="tip-icon-favorit"><FontAwesomeIcon icon={faEye} /></div>
                    <h4>Быстрый доступ</h4>
                    <p>Все понравившиеся дома в одном месте для удобного сравнения</p>
                  </div>
                  <div className="tip-card-favorit">
                    <div className="tip-icon-favorit"><FontAwesomeIcon icon={faCalendar} /></div>
                    <h4>Планирование</h4>
          ...
          <div className="favorites-header-favorit">
            <div className="favorites-hero-favorit">
              <div className="hero-content-favorit">
                ...
                <p className="favorites-subtitle-favorit">Все сохраненные дома в одном месте</p>
              </div>
              ...
              <div className="controls-left-favorit">
                <div className="selection-info-favorit">
                  {selectedItems.size > 0 ? (
                    <>
                      <span className="selected-count-favorit">Выбрано: {selectedItems.size} домов</span>
                      <button className="btn-clear-selection-favorit" onClick={() => setSelectedItems(new Set())}>
                        Снять выделение
                      </button>
                    </>
                  ) : (
                    <button className="btn-select-all-favorit" onClick={selectAllItems}>Выбрать все</button>
                  )}
                </div>
              </div>
            ...
            <div className="catalog-controls-favorit">
              ...
              <div className="controls-right-favorit">
                <div className="results-count-favorit">Показано: <strong>{sortedFavorites.length}</strong> домов</div>
              </div>
            </div>
          </div>
              ...
              <div key={property.id} className="property-item-favorit list-favorit">
                ...
                <div className="property-item-content-favorit">
                  <div className="property-item-header-favorit">
                    <h3>{formatPriceWithIcon(property.price, property.rentType)}</h3>
                  ...
                  <div className="property-item-features-favorit">
                    <span><FontAwesomeIcon icon={faBed} /> {property.rooms} комн.</span>
                    <span><FontAwesomeIcon icon={faBath} /> {property.bathrooms}</span>
                    <span><FontAwesomeIcon icon={faRulerCombined} /> {property.area} м²</span>
                    <span><FontAwesomeIcon icon={faCheckCircle} /> {property.floor} эт.</span>
                  </div>
                  <p className="property-item-description-favorit">{property.description}</p>
                  <div className="property-item-actions-favorit">
                    <Link to={`/house/${property.id}`} className="btn-primary-favorit">Подробнее</Link>
                    <button 
                    ...
                    >
                      {creatingChatForProperty === property.id ? (
                        <><FontAwesomeIcon icon={faSpinner} spin style={{ marginRight: '8px' }} /> Открытие чата...</>
                      ) : isAdmin ? (
                        <><FontAwesomeIcon icon={faComment} style={{ marginRight: '8px' }} /> Админам запрещено</>
                      ) : (
                        <><FontAwesomeIcon icon={faComment} style={{ marginRight: '8px' }} /> Написать владельцу</>
                      )}
                    </button>
                  </div>
                </div>
